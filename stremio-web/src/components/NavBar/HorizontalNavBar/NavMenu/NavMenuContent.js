...
const NavMenuContent = ({ onClick }) => {
    ...
    const handleAuth = React.useCallback(() => {
        // Kai: no Stremio cloud accounts — login/signup disabled
        if (profile.auth !== null) {
            logoutButtonOnClick();
        }
    }, [profile.auth, logoutButtonOnClick]);
            ...
            <div className={styles['user-info-container']}>
                ...
                <div className={styles['user-info-details']}>
                    <div className={styles['email-container']}>
                        <div className={styles['email-label']}>{profile.auth === null ? 'Kai (local)' : profile.auth.user.email}</div>
                    </div>
                    {
                        profile.auth !== null ?
